#!/bin/bash
#
# Assign different weights to make $Pr[Y_{true}|W_{avg}*X] \approx Pr[Y_{true}|X]$ as shown in Figure 11 of Appendix B
#

# Initial mitigation starts

convert $1.png -fx '(0.0625*(p[-3,-3] + p[-3,-2] + p[-3,-1] + p[-3,0] + p[-3,1] + p[-3,2] + p[-3, 3] + p[3,-3] + p[3,-2] + p[3,-1] + p[3,0] + p[3,1] + p[3,2] + p[3,3] + p[-2,-3] + p[-2,3] + p[-1,-3] + p[-1,3] + p[0,-3] + p[0,3] + p[1,-3] + p[1,3] + p[2,-3] + p[2,3]) + 0.25*(p[-2,-2] + p[-2,-1] + p[-2,0] + p[-2,1] + p[-2,2] + p[-1,-2] + p[-1,2] + p[0,-2] + p[0,2] + p[1,-2] + p[1,2] + p[2,-2] + p[2,-1] + p[2,0] + p[2,1] + p[2,2] ) + p[-1,-1] + p[-1,0] + p[-1,1] + p[0,-1] + p[0,0] + p[0,1] + p[1,-1] + p[1,0] + p[1,1])/14.5' $1_ma0.png
...
for i in {0..99}
do
	convert $1_mitigated_step$i.png -fx '(0.0625*(p[-3,-3] + p[-3,-2] + p[-3,-1] + p[-3,0] + p[-3,1] + p[-3,2] + p[-3, 3] + p[3,-3] + p[3,-2] + p[3,-1] + p[3,0] + p[3,1] + p[3,2] + p[3,3] + p[-2,-3] + p[-2,3] + p[-1,-3] + p[-1,3] + p[0,-3] + p[0,3] + p[1,-3] + p[1,3] + p[2,-3] + p[2,3]) + 0.25*(p[-2,-2] + p[-2,-1] + p[-2,0] + p[-2,1] + p[-2,2] + p[-1,-2] + p[-1,2] + p[0,-2] + p[0,2] + p[1,-2] + p[1,2] + p[2,-2] + p[2,-1] + p[2,0] + p[2,1] + p[2,2] ) + p[-1,-1] + p[-1,0] + p[-1,1] + p[0,-1] + p[0,0] + p[0,1] + p[1,-1] + p[1,0] + p[1,1])/14.5' $1_ma$((i+1)).png
